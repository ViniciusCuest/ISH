      ...
      <header className='w-full p-3 flex flex-row bg-sky-500 justify-between items-center z-50 rounded-b-2xl shadow-[0px_4px_5px_0px_#0000001c]'>
        <div className='flex flex-row gap-x-2'>
          ...
          <span>
            ...
            <p className='text-sm flex flex-row items-center gap-x-1 text-white underline font-light'>
              Editar meus dados <FaChevronRight size={10} />
            </p>
          </span>
        </div>
      ...
      <section className='px-5 flex flex-col'>
        ...
        <div className='flex flex-col space-y-2'>
          {[{ id: '1' }, { id: '2' }].map((item) => (
            <div
              key={item.id}
              className=' bg-zinc-100 w-full transition-all rounded-xl px-3 py-4 shadow-[0px_4px_5px_0px_#dadada36]'
              onClick={() => {
                setActive((prev) => {
                  if (prev.find((active) => active === item.id)) return prev.filter((active) => active !== item.id);
                  return [...prev, item.id];
                });
              }}
            >
              <div className='flex flex-row justify-between items-center'>
                <div>
                  <p className='text-base text-cyan-900'>
                    Pedido: <span className='uppercase font-medium text-sky-500'>AJ456789234</span>
                  </p>
                  <p className='text-sm text-cyan-900'>10/02/2024 às 14:40</p>
                </div>
                <p className='text-sm text-cyan-900'>
                  Status: <span className='font-medium'>Não visualizado</span>
                </p>
              </div>
              <div
                style={{
              ...
              >
                <div className='flex flex-row items-center justify-between'>
                  ...
                  <p className='text-sm'>
                    <span className='text-sky-500 font-semibold'>Feito por: </span>Vinicius Costa
                  </p>
                </div>
                <ul style={{ listStyle: 'inside', paddingLeft: 8, marginTop: 4 }}>
                  <li className='text-sm text-cyan-900'>Tainha (2kg) - R$ 10,39/kg</li>
                  <li className='text-sm text-cyan-900'>Camarão (20kg) - R$ 10,39/kg</li>
                </ul>
